...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!jsonl || typeof jsonl !== 'string') return NextResponse.json({ error: 'jsonl required' }, { status: 400 });
    const url = process.env.KV_REST_API_URL;
    const token = process.env.KV_REST_API_TOKEN;
    if (!url || !token) return NextResponse.json({ success: true, masterTotal: runTotal || 0, kvAvailable: false });
...
export async function GET() {
  try {
    ...
    if (!master) return NextResponse.json({ error: 'No data yet. Run the pipeline first.', total: 0 });
    const total = master.split('\n').filter(Boolean).length;
    return new NextResponse(master, { headers: { 'Content-Type': 'application/jsonl', 'Content-Disposition': `attachment; filename="sovereign-health-master-${total}-pairs.jsonl"`, 'X-Total-Pairs': String(total) } });
  } catch (err) { return NextResponse.json({ error: String(err) }, { status: 500 }); }
}
